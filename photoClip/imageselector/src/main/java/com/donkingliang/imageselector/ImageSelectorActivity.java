...
public class ImageSelectorActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (onlyTakePhoto) {
            ...
            setStatusBarColor();
            initView();
            initListener();//在Activity的onCreate()中调用finish()方法，则执行的生命周期方法顺序为：
            //onCreate() -> onDestroy()
            initImageList();
...
//也要修改
    ...
    private void openFolder() {
        if (!isOpenFolder) {
            ...
            ObjectAnimator animator = ObjectAnimator.ofFloat(rvFolder, "translationY",
                    rvFolder.getHeight(), 0).setDuration(300);//平移动画，改变Y轴坐标
    ...
    private void confirm() {
        ...
        for (Image image : selectImages) {
            images.add(image.getPath());/////采用Image.getPath()返回的
        }
    ...
    private void setResult(ArrayList<String> images, boolean isCameraImage) {
        ...
        intent.putStringArrayListExtra(ImageSelector.SELECT_RESULT, images);//以String返回的image
        intent.putExtra(ImageSelector.IS_CAMERA_IMAGE, isCameraImage);
        setResult(RESULT_OK, intent);//不是显式传回MainActivity,它的接口也是OK，放心了
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == ImageSelector.RESULT_CODE) {//如果start 里面request_code变了理论上这里也要变
    ...
    private void openCamera() {
        ...
        if (captureIntent.resolveActivity(getPackageManager()) != null) {
            ...
            if (photoUri != null) {
                ...
                startActivityForResult(captureIntent, CAMERA_REQUEST_CODE);//后一个参数相当于IntentType
